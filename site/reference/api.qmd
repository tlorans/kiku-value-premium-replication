...
Public names live at the package root. Submodules (`lrrcs.model`,
`lrrcs.empirical`, `lrrcs.calibration`, `lrrcs.implications`) exist for
organization but are not the documented path. The division of labor
mirrors the model's two halves: `calibrate_from_data` measures
cash-flow exposures (the cash-flow side), `solve_analytical` and
`compute_asset_pricing_moments` let the household price them (the
...
`solve_analytical` is the fast entry: the whole equilibrium in one
call. `ModelSolver` is the grid solver behind Table VII, and
...
`Dynamics`, `StateGrid`, `ModelSolver`, `resolve_legs` — the object
layer under the functions above; the flat functions are the documented
path.

A new sort is the same loop as
[Chapter 8](../chapters/08-calibrating-cash-flows.qmd): consumption and
two dividend legs in, no returns.
