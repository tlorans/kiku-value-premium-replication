"""Results of solving or simulating a :class:`~geap.CampbellCochraneModel`."""
...
class CampbellCochraneResults(AssetPricingResults):
    """Annualized moments of the external-habit economy, in percent."""
    ...
    def _subtitle(self) -> str:
        ...
        return "surplus grid, monthly, annualized (Campbell and Cochrane 1999)"

    def _header_lines(self, subtitle: str) -> list[str]:
        return [
            "=" * 72,
            "Campbell-Cochrane Habit Model Results".center(72),
            "=" * 72,
            "Model:        Campbell and Cochrane (1999) external habit",
            f"Method:       {subtitle}",
            f"Preferences:  gamma = {self.model.gamma:g}   "
            f"phi = {self.model.params.phi:g}",
    ...
    def summary(self) -> Summary:
        ...
        lines.append("-" * 72)
        lines.append(
            f"{'Claim':12s} {'E[R] %':>8s} {'Vol %':>8s} {'log(P/C)':>9s}"
        )
        vol = self.volatility
        ...
        lines.append("=" * 72)
        lines.append("Annualized from monthly returns.")
