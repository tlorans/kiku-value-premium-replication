...
class LongRunRisksModel(AssetPricingModel):
    ...
    def claims(self) -> tuple[str, ...]:
        return tuple(self.params.claims)
    ...
    def replace(self, **kwargs) -> "LongRunRisksModel":
        ...
        The one-liner for a sensitivity sweep::
            ...
            [model.replace(gamma=g).solve().compare("value", "growth").premium for g in (5, 10, 15)]
        """
    ...
    def solve(
        ...
        *,
        ...
        ```python
        import geap
        res = geap.LongRunRisksModel().solve()
        print(res.summary())
        ```
        """
        if method == "grid":
