...
The site is a Quarto website, and every chapter runs its Python at build time.
...
`site/_freeze`, and re-executes every chapter from scratch, so a change in the
...
pinned in code instead. `src/lrrcs/empirical/goldens.py` holds Kiku's printed
...
shipped data lands inside the standard errors she printed. A chapter that
quotes a paper figure says so in its own "Where each number comes from" block.

One caveat is open, and the site states it in
`site/chapters/09-the-market-test.qmd` and
`site/chapters/10-value-premium-resolved.qmd`. Kiku's printed Table VII return
...
her 0.92. The chapters print the package's own values next to hers rather than
...
prints its numbers rather than writing them anywhere. Two of them are cited by
a chapter, and the rest stand on their own.

| Script | What it prints | Cited by |
|---|---|---|
| `run_paper.py` | the whole paper, in the order Kiku presents it | — |
| `dcf_counterfactual.py` | the discounted cash flow counterfactual on the Table II economy, in three panels | chapter 1 |
| `robustness.py` | the model premium as one parameter at a time is varied | chapters 6 and 10 |
| `two_firms.py` | two synthetic firms differing only in their loading on the persistent component | — |
| `table_ii_calibration.py` | the Table II calibration, read from `lrr.ModelParams()` at run time | — |
...
volatility loading or the residual correlation. Chapter 10 now prints the real
figure and explains where the residual comes from.
...
log growth rates instead. Chapter 5 now shows both conventions side by side and
prints 0.43 under the level convention.
...
inputs and the solver's 37.5. Chapter 6 now computes it and mentions the old
misprint.
